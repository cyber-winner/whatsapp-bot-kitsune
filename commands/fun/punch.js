...
module.exports = {
  name: 'punch',
  aliases: [],
  description: 'Use punch on someone! Usage: -punch @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-punch @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['punch'];
    ...
    const mediaData = await fetchGif('punch');
