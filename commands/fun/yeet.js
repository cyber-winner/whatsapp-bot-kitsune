...
module.exports = {
  name: 'yeet',
  aliases: [],
  description: 'Use yeet on someone! Usage: -yeet @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-yeet @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['yeet'];
    ...
    const mediaData = await fetchGif('yeet');
