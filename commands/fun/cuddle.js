...
module.exports = {
  name: 'cuddle',
  aliases: [],
  description: 'Use cuddle on someone! Usage: -cuddle @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-cuddle @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['cuddle'];
    ...
    const mediaData = await fetchGif('cuddle');
