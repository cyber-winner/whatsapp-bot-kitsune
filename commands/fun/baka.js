...
module.exports = {
  name: 'baka',
  aliases: [],
  description: 'Use baka on someone! Usage: -baka @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-baka @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['baka'];
    ...
    const mediaData = await fetchGif('baka');
