...
module.exports = {
  name: 'poke',
  aliases: [],
  description: 'Use poke on someone! Usage: -poke @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-poke @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['poke'];
    ...
    const mediaData = await fetchGif('poke');
