...
module.exports = {
  name: 'carry',
  aliases: [],
  description: 'Use carry on someone! Usage: -carry @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-carry @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['carry'];
    ...
    const mediaData = await fetchGif('carry');
