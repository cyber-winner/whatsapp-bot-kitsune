...
module.exports = {
  name: 'shoot',
  aliases: [],
  description: 'Use shoot on someone! Usage: -shoot @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-shoot @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['shoot'];
    ...
    const mediaData = await fetchGif('shoot');
