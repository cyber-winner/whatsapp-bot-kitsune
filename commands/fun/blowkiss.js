...
module.exports = {
  name: 'blowkiss',
  aliases: [],
  description: 'Use blowkiss on someone! Usage: -blowkiss @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-blowkiss @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['blowkiss'];
    ...
    const mediaData = await fetchGif('blowkiss');
