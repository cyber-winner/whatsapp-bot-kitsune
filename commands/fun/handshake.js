...
module.exports = {
  name: 'handshake',
  aliases: [],
  description: 'Use handshake on someone! Usage: -handshake @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-handshake @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['handshake'];
    ...
    const mediaData = await fetchGif('handshake');
