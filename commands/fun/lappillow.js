...
module.exports = {
  name: 'lappillow',
  aliases: [],
  description: 'Use lappillow on someone! Usage: -lappillow @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-lappillow @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['lappillow'];
    ...
    const mediaData = await fetchGif('lappillow');
