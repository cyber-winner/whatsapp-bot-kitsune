...
module.exports = {
  name: 'bite',
  aliases: [],
  description: 'Use bite on someone! Usage: -bite @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-bite @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['bite'];
    ...
    const mediaData = await fetchGif('bite');
