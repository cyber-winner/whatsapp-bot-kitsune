...
module.exports = {
  name: 'handhold',
  aliases: [],
  description: 'Use handhold on someone! Usage: -handhold @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-handhold @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['handhold'];
    ...
    const mediaData = await fetchGif('handhold');
