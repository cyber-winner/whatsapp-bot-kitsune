...
module.exports = {
  name: 'peck',
  aliases: [],
  description: 'Use peck on someone! Usage: -peck @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-peck @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['peck'];
    ...
    const mediaData = await fetchGif('peck');
