...
module.exports = {
  name: 'hug',
  aliases: [],
  description: 'Use hug on someone! Usage: -hug @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-hug @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['hug'];
    ...
    const mediaData = await fetchGif('hug');
