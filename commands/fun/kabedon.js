...
module.exports = {
  name: 'kabedon',
  aliases: [],
  description: 'Use kabedon on someone! Usage: -kabedon @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-kabedon @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['kabedon'];
    ...
    const mediaData = await fetchGif('kabedon');
