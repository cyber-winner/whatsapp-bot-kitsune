...
module.exports = {
  name: 'highfive',
  aliases: [],
  description: 'Use highfive on someone! Usage: -highfive @user',
  adminOnly: false,
  async execute(msg, args, client) {
    ...
    if (msgMentions.length === 0) {
      return msg.reply('❌ *Usage:* `-highfive @user`\n\n_Mention someone!_');
    }
    target = msgMentions[0];
    targetName = getDisplayName(target);
    mentions.push(target);
    const phrases = require('../../data/phrases.json').fun['highfive'];
    ...
    const mediaData = await fetchGif('highfive');
